...
class ProductUtils {
  static async findProductById(productId) {
    try {
      ...
      throw Error(error.message || 'Failed to find product by ID.');
    }
  }
  ...
  static async findProductsByName(name) {
    try {
      ...
      if (!products) {
        throw new Error('No products found with the given name.');
      }
  ...
  static async fetchAllProducts(page = 1, limit = 10) {
    try {
      ...
      console.error(
        'Error fetching all products with pagination:',
        error.message
      );
